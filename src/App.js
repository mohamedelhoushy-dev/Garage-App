  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
      <div style={{ background: "#0d1020", border: "1px solid #1e2340", borderRadius: 16, padding: "28px 24px", width: 340, color: "#e0e4f0" }}>
        ...
        <input placeholder="Owner Name" value={form.owner} onChange={e => setField("owner", e.target.value)} style={{ width: "100%", marginBottom: 10, padding: 10, borderRadius: 7, background: "#161928", border: "1px solid #1e2340", color: "white", boxSizing: "border-box" }} />
        <select value={form.brand} onChange={e => setField("brand", e.target.value)} style={{ width: "100%", marginBottom: 10, padding: 10, borderRadius: 7, background: "#161928", border: "1px solid #1e2340", color: "white", boxSizing: "border-box" }}>
          <option value="">Select Brand</option>
          {CAR_BRANDS.map(b => <option key={b} value={b}>{b}</option>)}
        </select>
        <input placeholder="License Plate" value={form.plate} onChange={e => setField("plate", e.target.value)} style={{ width: "100%", marginBottom: 10, padding: 10, borderRadius: 7, background: "#161928", border: "1px solid #1e2340", color: "white", boxSizing: "border-box" }} />
        <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
          {SLOT_COLORS.map(c => (
            <div key={c} onClick={() => setField("color", c)}
              style={{ width: 24, height: 24, borderRadius: "50%", background: c,
                border: form.color === c ? "3px solid white" : "2px solid transparent", cursor: "pointer" }} />
          ))}
        </div>
        <input type="datetime-local" value={form.departure} onChange={e => setField("departure", e.target.value)} style={{ width: "100%", marginBottom: 20, padding: 10, borderRadius: 7, background: "#161928", border: "1px solid #1e2340", colorScheme: "dark", boxSizing: "border-box" }} />
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={onClose} style={{ flex: 1, padding: 10, background: "transparent", color: "#3a4060", border: "1px solid #1e2340", borderRadius: 7, cursor: "pointer" }}>Cancel</button>
          <button onClick={() => onSave(slot.id, form)} style={{ flex: 1, padding: 10, background: "#e0e4f0", color: "#0d1020", borderRadius: 7, fontWeight: "bold", cursor: "pointer" }}>Save</button>
          {slot.occupied && <button onClick={() => onSave(slot.id, null)} style={{ padding: 10, background: "#3a1a1a", color: "#e05252", border: "1px solid #e05252", borderRadius: 7, cursor: "pointer" }}>Exit</button>}
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ height: "100vh", background: "#080b14", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontFamily: "sans-serif" }}>
        <div style={{ textAlign: "center", padding: 20, background: "#0d1020", borderRadius: 16, border: "1px solid #1e2340" }}>
          ...
          <input
            ...
            style={{ padding: 12, borderRadius: 8, border: "1px solid #1e2340", background: "#161928", color: "white", textAlign: "center", fontSize: 20, width: 120, marginBottom: 20 }}
          />
          <br />
